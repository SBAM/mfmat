namespace mfmat
{
  ...
  ct_mat<T, R, C> orthonormalize(const ct_mat<T, R, C>& arg) noexcept
  {
    ...
    auto substract_proj = [&]<auto N, auto K>(decl_ic<N>, decl_ic<K>)
      {
        auto scale = dot<op_way::col, K, op_way::col, N>(res, arg);
        substract_scaled(decl_ic<N>{},
                         decl_ic<K>{},
                         scale, make_index_range<0, R>());
      };
    ...
    // processes a single column, copy it to output, apply projections and
    // normalize it
